#!/usr/bin/env bash
# Greenfield scaffold: git-flow governance base, then the python overlay,
...
# every post-gen task (uv python pin / uv sync / git init / lefthook
# install), leaving no lock file and no hooks.
# Existing repos use retrofit-governance.sh instead.
#
# usage: scripts/py-new.sh <new-project-dir>
set -euo pipefail
...
uvx copier copy --trust --overwrite "$template_repo_dir/python" "$1"
