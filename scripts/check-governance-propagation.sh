...
# tooling and the shipped git-flow/python template's mirrored copy, or the
...
template_pattern='^(git-flow|python)/template/.*(\.github/workflows/.*|lefthook.*\.ya?ml|justfile.*|\.editorconfig|\.gitleaks\.toml|\.markdownlint-cli2\.ya?ml|\.yamlfmt|\.prettierrc\.json|cliff\.toml.*)$'
...
if [ "$touched_root" -eq 1 ] && [ "$touched_template" -eq 0 ]; then
  echo "check-governance-propagation: this branch touches this repo's own governance surface but not git-flow/python template's — evaluate propagating to git-flow/template/ (and python/template/ if relevant)"
